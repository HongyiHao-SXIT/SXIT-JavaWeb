...
<script type="text/javascript">
	function deleteCart(obj){
		if(window.confirm("确认删除吗？")){
			...
			window.location.href = projectName + "/before_cart?act=delete&id=" +  obj;
		}
	}
	function clearCart(){
		if(window.confirm("确认清空吗？")){
			...
			//截取，得到项目名称
			var projectName=pathName.substring(0,pathName.substr(1).indexOf('/')+1);
			window.location.href = projectName + "/before_cart?act=clear";
		}
	}
</script>
</head>
				...
				<table class="table table-bordered table-hover">
					<tbody class="text-center">
						...
						<c:if test="${cartlist.size() > 0 }">
							...
							<tr>
								<td>
									<a href="before_detail?gno=${cart.id}"> 
									<img src="static/images/${cart.gpicture}"
										style="height: 50px; width: 50px; display: block;">
									</a>
								</td>
								<td>
									<a href="before_detail?gno=${cart.id}"> ${cart.gname }</a>
								</td>
								<td>${cart.grprice}</td>
								<td>${cart.shoppingnum}</td>
								<td>${cart.smallsum}</td>
								<td>
									<a href="javascript:deleteCart(${cart.id})" >删除</a>
								</td>
							</tr>
							</c:forEach>
							<tr>
							...
							<tr>
								<td colspan="6">
									<a href="before_cart?act=toCount" >去结算</a>
								</td>
							</tr>
						</c:if>
					</tbody>
