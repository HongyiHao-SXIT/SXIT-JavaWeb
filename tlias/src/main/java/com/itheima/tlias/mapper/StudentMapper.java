...
public interface StudentMapper {
    ...
    @Select("SELECT c.name AS clazzName, COUNT(*) AS studentCount " +
            "FROM student s JOIN clazz c ON s.clazz_id = c.id " +
            "GROUP BY s.clazz_id")
    List<Map<String, Object>> countByClazz();

    @Select("SELECT " +
            "CASE degree WHEN 1 THEN '初中' WHEN 2 THEN '高中' " +
            "WHEN 3 THEN '大专' WHEN 4 THEN '本科' " +
            "WHEN 5 THEN '硕士' WHEN 6 THEN '博士' END AS degreeName, " +
            "COUNT(*) AS count FROM student GROUP BY degree")
